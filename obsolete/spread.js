...
// No filtering of text to be read is done in spread.js
...
function processText(text, settings) {
    ...
    var textScope = new Element('div', { // confirm removing new has no effect to the problem of div beng properly created with method of makeResizable
        id: "textScope_panel"
	      //,
        // style: "width:30%; float:right; height:97%;" // format("width:30%;float:right;height:{0}px;", wsize.y) 
        });

    // textScope.makeResizable().detach(); // actually make it not resizable

    // var reader_inner_container = Element('div', {
    //         id: "textScope_body_container"
    //     });
...
var TEXTSCOPE_TEXT_READER = function(content, conf){
    ...
    this.aSpeedController.update(textHasChinese);
    ...
    this.wordsTotal = this.wordsCount(content);
    this.next = function(){
         var c = this.content[this.idx]; // use the decomposed segments, not just the space seperated words.
        // this.idx += this.step;
        this.idx ++;
    ...
    this.checkByInterval = function(){
        ...
        if ( this.intervalsToWait <= 0) {
            ...
            this.intervalsToWait = this.wordsCount(hede);
            this.wordsFlashed += this.intervalsToWait;
            flush(this.idx);
    ...
    this.flush = function(current) {
        ...
        var addOneSegmentLine = function (seg, index, segs) {
            ...
            var fontSize = (current) ? (((index + 1)*1.5/segs.length)*100 + '%') : '100%';
            reader_flush.setStyle('font-size', fontSize);
            var fontWeight = (current) ? (((index - 1) == segs.length) ? 'bold' : 'lighter') : 'normal';
            reader_flush.setStyle('font-weight', fontWeight);
            reader_flush.innerHTML = "<pre>" + seg + "</pre>"; // preserve the newlines using preformated tag
            // seg = seg.replace(/\n/g, "<br/>");
        ...
        var previous = (current - 2) > 0 ? (current - 2) : 0;
        var toShow = (current) ? this.content.slice(previous, current) : this.content;
        toShow.forEach(addOneSegmentLine);
        // $('textScope_flushes').focus();
        // if (current) 
        //     this.content.slice(previous, current).forEach(addOneSegmentLine) 
        // else
        //     this.content.forEach(addOneSegmentLine);
    ...
    this.updateSettings = function(){
        settings = {
           ...
           'font': parseInt($('textScope_flushens').getStyle('font-size'))
        };
