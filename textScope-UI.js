...
('mouseup',
  function(event){
	  ...
	  var text = getSelectedParagraphText();
          if (text.length){
              if (!notifier){
    ...
    var background = new Element('div', {
        ...
        events: {
            ...
            'keydown': function(event){
                ...
                return event; // there is no callback for function (event)
            }
        }
    }
                                );
    ...
    var textScope_flushes = Element('div', {
        ...
        events: {
            'click': function(event) {reader.pauseResumeToggle();
                                      return event;},
            'keydown': function(event) {if (reader.running == true) 
                                        {reader.pauseResumeToggle();
...
var TEXTSCOPE_TEXT_READER = function(content, conf){
    ...
    this.wordsCount = function(text) {
        ...
        // The following expression's meaning is all words of alphebets \w+ and all those non-punctuations, non-newline, etc. 
        var charArray=text.match(/\w+|[^.,\uFF10-\uFF19, \uFF9E, \uFF9F -\/#!$%\^&\*;:{}=\-_`~()\n]/g); 
    ...
    this.flush = function(start, end) {
        ...
        var addOneSegmentLine = function (seg, index, segs) {
                ...
                reader_flush = Element('div', 
                                       {
                                           ...
                                           styles: {
                                               'font-size': '24px',
                                               //'font-size': fontSize, // '1.5em', // experiment variable font size
                                               //'font-weight': '900',
                                               //'margin-top': '15px',
                                               'margin-bottom': '5px',
                                               'margin-left': '5px',
            ...
            reader_flush.innerHTML = "<pre id=preFormat>" + seg.text + "</pre>"; // preserve the newlines using preformated tag
            // seg = seg.replace(/\n/g, "<br/>");
        ...
        var previous = (start - 1) > 0 ? (start - 1) : 0; // no more previous line, which is too confusing
